  ...
  def on_epoch_begin(self, epoch, logs={}):
    if epoch > 0 and epoch % self.eval_frequency == 0:
      ...
      if len(checkpoints) > 0:
        ...
        lstm_model = model.compile_model(lstm_model, self.learning_rate)
        loss, acc = lstm_model.evaluate_generator(
            model.generator_input(self.eval_files, chunk_size=CHUNK_SIZE),
            steps=self.steps)
        print('\nEvaluation epoch[{}] metrics[{:.2f}, {:.2f}] {}'.format(
            epoch, loss, acc, lstm_model.metrics_names))
...
def dispatch(train_file,
  ...
  if job_dir.startswith("gs://"):
    ...
    copy_file_to_gcs(job_dir, LSTM_MODEL)
  else:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
        ...
        type=int,
        default=10,
        help='Number of hidden units in LSTM layer 1')
  parser.add_argument(
        '--lstm2_nodes',
        type=int,
        default=10,
        help='Number of hidden units in LSTM layer 2')
  parser.add_argument(
        '--mlp1_nodes',
        type=int,
        default=10,
        help='Number of hidden units in MLP layer 1 (after LSTM layer 2)')
